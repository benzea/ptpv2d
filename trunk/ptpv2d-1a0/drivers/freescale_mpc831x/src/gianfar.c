...
/*                                                                          */
/* This file (gianfar.c) contains Modifications (updates, corrections       */
...
int startup_gfar(struct net_device *dev)
{
	...
	if (priv->rx_csum_enable) {
		printk(KERN_INFO"%s: Rx CSUM Enable\n",dev->name);
	...
	if (priv->vlan_enable) {
		printk(KERN_INFO"%s: VLAN Support Enable\n",dev->name);
		rctrl |= RCTRL_VLAN;
	}
	if (priv->padding) {
		printk(KERN_INFO"%s: Rx Padding Enable\n",dev->name);
...
#if defined(CONFIG_GFAR_PTP)
	int timestamp_id; /* AKB: Message type and sequence number for storing into timestamp ID */
#endif
...
#if defined(CONFIG_GFAR_PTP)
	/* AKB: Check if timestamp needed (-1 if not) */
	timestamp_id = gfar_ptp_do_txstamp(skb);
	if (timestamp_id != -1) {
		/* AKB: Timestamp needed, get fcb if not already
		 * gotten, set PTP flag, set timestamp ID
		 * (same place as VLAN control if PTP enabled)
		 * to timestamp ID returned from gfar_ptp_do_txstamp
		 * (which consists of PTP message type and least
		 * significant 8 bits of the PTP sequence number for
		 * that message type) and set TOE flag in status variable
		 */
		if (fcb == NULL) {
			fcb = gfar_add_fcb(skb, txbdp);
		}
