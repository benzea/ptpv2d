              ...
              )
{
  ...
  if (current_timer_add_value == 0) // Check if timer has been initialized
  {
    // Add value is zero, so MPC831X timer has not been set
    if (rtOpts->hwClockPeriod > 0)  // 0 == use default, else set the HW clock period
    {
       DBGV("doInit: setting Clockperiod %d nanoseconds\n", rtOpts->hwClockPeriod);
       mpc831x_set_period(
                          rtOpts->hwClockPeriod
                         );
    }
    else
    {
       DBGV("doInit: setting Clockperiod %d nanoseconds\n", 7);
       mpc831x_set_period( 
                          7               // Default 7 nanosecond period for max performance
                         );
    }
  }
  else
  {
    DBGV("doInit: current_timer_add_value already set to %d\n", current_timer_add_value);
  }
#endif
...
#ifdef CONFIG_MPC831X
  // Set HW clock start time from Operating system clock
  setPtpTimeFromSystem(rtOpts->currentUtcOffset); 
#endif
            ...
            )
{
  ...
  case PTP_SLAVE:
    ...
    //
#ifdef CONFIG_MPC831X
    // Check if Delay request or PDelay Request outstanding)
    checkTxCompletions(rtOpts, ptpClock);
#endif
...
#ifdef CONFIG_MPC831X
    /* Clear Yellow LED if set and set meter to max */
    yellow_alarm(TRUE);
    led_meter(255);
#endif
...
#ifdef CONFIG_MPC831X
    /* Set Yellow and Red LEDs and set meter to max */
    yellow_alarm(TRUE);
    red_alarm(TRUE);
    led_meter(255);
#endif
...
#ifdef CONFIG_MPC831X
    /* Set Green LED (also clears Yellow LED) and set meter to max */
    green_alarm(TRUE);
    led_meter(255);
#endif
...
#ifdef CONFIG_MPC831X
    /* Clear Yellow and Green LED and set meter to max */
    yellow_alarm(FALSE);
    green_alarm(FALSE);
    led_meter(255);
#endif
    ...
    if(!length)
    {
      ...
      else
      {
        if(!length)
        {
          ...
          // Check if 802.1AS is enabled and if so, check that
          // socket has some data to process
          if (rtOpts->ptp8021AS)
          {
            // 802.1AS enabled, check if anything available from the 
            // raw socket
          ...
          {
             // No 802.1AS and no frames to process, return
             return;
          }
        }
      }
...
#ifdef CONFIG_MPC831X
  /* AKB: Updated code for timestamp, changed for updated Freescale Ethernet
   * driver code and also to put everything in one place for easier future
   * updates in the ptpd code
   */
  
  if (isFromSelf)
  {
     /* Frame is from self (looped back through socket)
      * Check if it is a SYNC or DELAY REQUEST, if so
      * get the transmit timestamp from the hardware
      */
      switch(ptpClock->v2_msg_type)
      {
        ...
        case V2_PDELAY_RESP_MESSAGE: /* V2 type: 0x3 */
           ...
           mpc831x_get_tx_time(&ptpClock->netPath, 
                               &time,
                                current_sequence,
                                ptpClock->v2_msg_type
                              );

           /* BEGIN DEBUG TEMP FOR LOOPBACK TEST */

TimeInternal debug_rx_time    = {0,0};
TimeInternal debug_delta_time = {0,0};

           mpc831x_get_rx_time(&ptpClock->netPath,
                               &debug_rx_time,
                               current_sequence,
                               ptpClock->v2_msg_type
                              );

           subTime(&debug_delta_time, &debug_rx_time, &time);

           printf("%u,%u.%9.9u,%u.%9.9u,%u.%9.9u\n",
                  current_sequence,
                  time.seconds,
                  time.nanoseconds,
                  debug_rx_time.seconds,
                  debug_rx_time.nanoseconds,
                  debug_delta_time.seconds,
                  debug_delta_time.nanoseconds
                 );

           /* END DEBUG TEMP FOR LOOPBACK TEST */
  ...
  {
     /* Frame is not from self (actually received from outside world)
      * Check if it is a SYNC or DELAY REQUEST, if so
      * get the receive timestamp from the hardware
      */
      switch(ptpClock->v2_msg_type)
      {
        case V2_SYNC_MESSAGE:        /* V1 control: 0x00, V2 type: 0x0 */
...
Boolean      state_ok;
  ...
  if (!ptpClock->tx_time_pending)
  {
    DBGV("checkTxCompletions: no tx timestamps pending\n");
    return;
  }

  DBGV("checkTxCompletions: at least one tx timestamp pending\n");

  /* Sync, Delay request, PDelay request or PDelay response pending */
  if (ptpClock->tx_sync_time_pending)
  {
    DBGV("checkTxCompletions: sentSync flag is TRUE\n");
    /* Sync time pending set, make sure we are still in MASTER state) */
    if (ptpClock->port_state == PTP_MASTER)
    {
      /* Sync time pending is set and we are master,
       * try and poll for time from driver
       */
      mpc831x_get_tx_time(&ptpClock->netPath, 
                          &time,
      ...
      DBGV("checkTxCompletions: HW time %d.%9.9d, sequence %d\n",
           time.seconds,
           time.nanoseconds,
           ptpClock->last_sync_tx_sequence_number
          );
      ...
      {
         /* Time value is not 0, TX frame timestamp has been captured and is valid */
    ...
    {
      /* Sync Tx pending is true, but we are no longer master, clear sentSync
       * and time pending Flags
       */
      ptpClock->sentSync = FALSE;
      ptpClock->tx_sync_time_pending = FALSE;
    }
  ...
  if (ptpClock->tx_delay_req_time_pending)
  {
    DBGV("checkTxCompletions: delay request tx time pending is TRUE\n");
    /* Delay Request time pending set, 
     * check if we are sending Delay Requests or PDelay Requests
     */
    if (rtOpts->pdelay)
    {
      DBGV("checkTxCompletions: checking PDelay Request tx completion\n");
      message_type = V2_PDELAY_REQ_MESSAGE;
      state_ok     = (ptpClock->port_state >= PTP_LISTENING);
    }
    else
    {
      DBGV("checkTxCompletions: checking Delay Request tx completion\n");
      message_type = V2_DELAY_REQ_MESSAGE;
      state_ok     = (ptpClock->port_state == PTP_SLAVE);
    }
    if (state_ok)
    {
      /* Delay Request time pending Flag is set and state is OK,
       * try and poll for time from driver
       */
      mpc831x_get_tx_time(&ptpClock->netPath, 
                          &time,
                          ptpClock->last_delay_req_tx_sequence_number,
                          message_type
                         );
      ...
      if (isNonZeroTime(&time))
      {
         ...
         handleDelayReqTxComplete(&time, rtOpts, ptpClock);
         ptpClock->tx_delay_req_time_pending = FALSE;
      }
    }
    else
    {
      /* sentDelayReq is true, but state is no longer OK, clear sentDelayReq Flag */
      ptpClock->sentDelayReq = FALSE;
      ptpClock->tx_delay_req_time_pending = FALSE;
    }
  }

  if (ptpClock->tx_pdelay_resp_time_pending)
  {
    DBGV("checkTxCompletions, PDelay Response tx time pending flag is TRUE\n");

    if (ptpClock->port_state >= PTP_LISTENING)
    {
      /* PDelay Response time pending and state is OK, try and poll for time from driver */

      mpc831x_get_tx_time(&ptpClock->netPath, 
                          &time,
                          ptpClock->last_pdelay_resp_tx_sequence_number,
      ...
      {
         /* Time value is not 0, TX frame timestamp has been captured and is valid */
         handlePDelayRespTxComplete(&time, rtOpts, ptpClock);
         ptpClock->tx_pdelay_resp_time_pending = FALSE;
      }
    }
    else
    {
      /* sentDelayReq is true, but state is no longer OK, clear PDelay Response Flags */
  ...
  if (!(   ptpClock->tx_sync_time_pending 
        || ptpClock->tx_delay_req_time_pending
        || ptpClock->tx_pdelay_resp_time_pending
       )
     )
  {
    ptpClock->tx_time_pending = FALSE;

    /* Clear poll timeout so we go back to long waits and
     * handle the transmit complete for the Sync message.
     */
    clearTime(&ptpClock->tx_complete_poll_timeout);
  }
  else
  {
    /* Some frame type is still pending:
     * Setup poll timeout so we can check it again by calling select
     * with a timeout value.  We want to be timely, but we don't
     * want to fully overburden the operating system if the packet gets stuck
     * behind several others, so we set the initial timeout value
     * to 1/2 millisecond and add from there.
     */
    ptpClock->tx_complete_poll_timeout.seconds         =        0;

    ptpClock->tx_complete_poll_timeout.nanoseconds     +=  500000;

    if (ptpClock->tx_complete_poll_timeout.nanoseconds >= 5000000)
    {
      /* Too many timeouts, assume Transmit completions were lost */
      ...
      ptpClock->tx_pdelay_resp_time_pending = FALSE;
      ptpClock->sentSync                    = FALSE;
      ptpClock->sentDelayReq                = FALSE;  // Used for both Delay & PDelay request
      ptpClock->sentPDelayResp              = FALSE;
      ptpClock->receivedPDelayResp          = FALSE;
    }
  }
                   ...
                   )
{
  ...
  if (
         ((ptpClock->current_msg_version == 1) && (length < FOLLOW_UP_PACKET_LENGTH))
...
#ifdef CONFIG_MPC831X
      if(ptpClock->tx_delay_req_time_pending == TRUE)
      {
         ptpClock->tx_delay_req_time_pending = FALSE;
         if (!(ptpClock->tx_sync_time_pending || ptpClock->tx_pdelay_resp_time_pending))
         {
           clearTime(&ptpClock->tx_complete_poll_timeout);
         }
      }
...
#ifdef CONFIG_MPC831X
      if(ptpClock->tx_pdelay_resp_time_pending == TRUE)
      {
         ptpClock->tx_pdelay_resp_time_pending = FALSE;
      }
#endif
                  ...
                  )
{
  ...
  if (rtOpts->ptp8021AS)
  {
     DBGV("issueAnnounce: netSendRaw, sequence: %d\n",
          ptpClock->last_announce_tx_sequence_number
         );
...
#ifdef CONFIG_MPC831X
  ptpClock->tx_time_pending             = TRUE;
  ptpClock->tx_sync_time_pending        = TRUE;
#endif
                  ...
                  )
{
  ...
  if (rtOpts->ptp8021AS)  // AKB: Check if 802.1AS mode, if so, use raw socket
  {
     ret = netSendRaw(&(ptpClock->outputBuffer[2]),
                      (packet_length+14),
                      &ptpClock->netPath
...
#ifdef CONFIG_MPC831X
  ptpClock->tx_time_pending           = TRUE;
  ptpClock->tx_delay_req_time_pending = TRUE;
#endif
